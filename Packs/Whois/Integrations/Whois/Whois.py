...
grammar = {
    "_data": {
        'id': ['Domain ID:[ ]*(?P<val>.+)'],
        r'status': ['\[Status\]\s*(?P<val>.+)',
                   r'Status\s*:\s?(?P<val>.+)',
                   r'\[State\]\s*(?P<val>.+)',
                   r'^state:\s*(?P<val>.+)'],
        r'creation_date': ['\[Created on\]\s*(?P<val>.+)',
        ...
        r'expiration_date': ['\[Expires on\]\s*(?P<val>.+)',
                            ...
                            r'renewal:\s*(?P<val>.+)',
                            r'expire:\s*(?P<val>.+)'],
        r'updated_date': ['\[Last Updated\]\s*(?P<val>.+)',
        ...
        r'registrar': ['registrar:\s*(?P<val>.+)',
                      ...
                      r'\tName:\t\s(?P<val>.+)'],
        r'whois_server': ['Whois Server:\s?(?P<val>.+)',
        ...
        'nameservers': ['Name Server:[ ]*(?P<val>[^ ]+)',
                        ...
                        r'Nserver:\s*(?P<val>.+)'],
        r'emails': ['(?P<val>[\w.-]+@[\w.-]+\.[\w]{2,6})',  # Really need to fix this, much longer TLDs now exist...
...
def get_whois_raw(domain, server="", previous=None, never_cut=False, with_server_list=False,
    ...
    if is_recursive:
        for line in [x.strip() for x in response.splitlines()]:
            match = re.match(r"(refer|whois server|referral url|registrar whois(?: server)?):\s*([^\s]+\.[^\s]+)", line,
                             re.IGNORECASE)
...
def parse_registrants(data, never_query_handles=True, handle_server=""):
    ...
    for obj in (registrant, tech_contact, billing_contact, admin_contact):
        if obj is not None:
            ...
            if 'country' in obj and 'city' in obj and (re.match(r"^R\.?O\.?C\.?$", obj["country"], re.IGNORECASE) or obj[

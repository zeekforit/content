...
def query_datalake(self, start_time: int = None, query: str = None):
    ...
    start_time = demisto.args().get('startTime', 1)
    search_query = {
        "sort": [
            {
                "indexTime": "asc"
    ...
    response = self._http_request("POST", full_url=f'{self._base_url}/dl/api/es/search', params=params2, headers=headers, timeout=120, resp_type='text')
...
def main():
    ...
    commands = {
        ...
        'exabeam-datalake-query': query_datalake
    }
...
from soar.library.common.context_tables_connector_mixin import ContextTablesConnectorMixin
from soar.utility.decorators import generic_exception_handler
from soar.utility.soar_session import SoarSession


class ExabeamDL(ContextTablesConnectorMixin):
    def __init__(self, host, port, username, password):
        self._session = SoarSession(base_url=f'https://{host}:{port}',
                                    headers={'kbn-version': '5.1.1-SNAPSHOT'},
                                    timeout=120,
                                    verify=False)
    ...
    def test_connection(self):
        response = self._session.post('api/auth/login', json=self._auth)
        response.raise_for_status()
        return True

    @generic_exception_handler
    def submit_query(self, input_query, start_time):
        search_query = {
            "sort": [
                {
                    "indexTime": "asc"
        ...
        response = self._session.post('data/elasticsearch/exabeam-*/_search', params={'size': 200}, json=search_query)
        response.raise_for_status()
        return response.json()['hits']['hits']
